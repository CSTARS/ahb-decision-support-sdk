...
function addYearData(results, yearRotation) {
  ...
  for( var i = 0; i < yearRotation.length; i++ ) {
    ...
    if( d.transportation ) {
      // 1 is for loading / unloading cost
      // * 2.32 because it's wet tons
      var transportationCost = d.transportation * (2.32 * d.yield) + (1 * d.yield);
