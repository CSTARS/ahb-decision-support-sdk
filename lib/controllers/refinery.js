...
function RefineryController() {
  ...
  events.on('get-refinery-parameters', (e) => {
    e.handler({
      lat: this.lat,
      lng: this.lng,
      radius : this.radius
  ...
  events.on('optimize-refinery', (options) => {
...
function ModelRunner(options, callback) {
  ...
  this.run = function() {
    ...
    collection.select(options.refinery, options.ROR, options.maxPastureLand);
  ...
  this.runParallelRequests = function() {
    ...
    transportationCollection.load(loc, options.routeGeometry, this.onParallelRequestComplete.bind(this));
