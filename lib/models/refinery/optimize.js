...
function onDataPrepared(refineryCollection, options, prepareResults, callback) {
    ...
    prepareResults.sortList.forEach((props) => {
        if( props.pastureIgnore ) {
            var parcel = parcelController.collection.get(props.id);
            parcel.properties.ucd.pastureIgnored = true;
            parcelController.collection.update(parcel);
            return;
        }

        if( refineryFull ) return;
...
function postProcess(refineryCollection, callback) {
    ...
    async.eachSeries(
        ...
        (id, next) => {
            ...
            var parcel = parcelController.collection.get(id);
            var growthProfile = growthProfilesCollection.get(parcel.properties.ucd.modelProfileId);
