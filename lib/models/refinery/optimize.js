...
var growthProfilesCollection = require('../../collection/growthProfiles');
var parcelController = require('../../controllers/parcels');
var transportationController = require('../../controllers/transportation');
...
function onDataPrepared(prepareResults, callback) {
    ...
    transportationController.clearCost();
    ...
    async.eachSeries(
        ...
        if( results.yield > requiredYield ) {
            this.poplarPrice = parseFloat((poplarPrice+0.01).toFixed(2));
            parcelController.collection.mwa = this.poplarPrice;
        ...
        parcelController.collection.get(props.id, (parcel) => {
            growthProfilesCollection.get(parcel.properties.ucd.modelProfileId, (growthProfile) => {
                ...
                transportationController.sumCost(parcel);
        ...
        (err) => {
            onComplete(results, this, callback);
        }
    );
}

function onComplete(results, model, callback) {
    // price not found
    if( results.capacityReachedAt === -1 ) {
        parcelController.collection.poplarPrice = model.collection.maxWillingToPay;
