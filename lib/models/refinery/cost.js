...
function prepare(parcel, growthProfile, datastore) {
    if( !growthProfile ) return;

    var ucd = parcel.properties.ucd;

    var poplarCycle = createPoplarCycle(parcel, growthProfile, datastore);
...
function setCropCycle(ucd, datastore) {
    ...
    for( var i = 0; i < ucd.cropInfo.swap.length; i++ ) {
        ...
        budget = datastore.budgets[ucd.budgetIds[i]];
...
function createPoplarCycle(parcel, growthProfile, datastore) {
    var ucd = parcel.properties.ucd;
