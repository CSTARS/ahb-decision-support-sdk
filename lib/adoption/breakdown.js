...
module.exports = function(datastore, farmBudgetSdk) {
  return function(minPrice, maxPrice, step, callback) {
    ...
    // TODO: lets calculate price of refinery here.
    // First price to draw a profit of 10% ROI wins?
    //var totalCost = calculateRefineryCost(datastore);
    ...
    async.eachSeries(
      ...
      function(price, next) {
        ...
        if( minPoplarPrice === null && (result.poplar.yield >= requiredYield || price === maxPrice) ) {
          ...
          console.log(totalCostAtPrice+' '+income);
