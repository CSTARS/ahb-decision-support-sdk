...
function calculateRevenue(args, datastore) {
    ...
    var poplarIncome, cropNet, poplarNet, parcel, ucd, data;

    for( var i = 0; i < datastore.validParcels.length; i++) {
        parcel = datastore.validParcels[i];
        ucd = parcel.properties.ucd;
        
        if( parcel.properties.PolyID === '345000064666' && args.price == 160 ) {
          debugger;
        } 
        ...
        if( data.netDifference > 0 && data.averagePoplarNet > 0 ) {
          data.adopted = true;
