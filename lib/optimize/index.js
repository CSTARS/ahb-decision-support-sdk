...
module.exports = function(datastore, farmBudgetSdk, poplarModel) {
  var v2 = require('../adoption/v2')(datastore, farmBudgetSdk, poplarModel);
...
function calculateRevenue(args, datastore) {
    ...
    for( var i = 0; i < datastore.validParcels.length; i++) {
        ...
        data = {
          ...
          averagePoplarProfit : calcAveragePoplarProfit(poplarIncome.yearly, ucd.farmCost.poplar.yearlyData),
          adopted : false
        };
        
        result.data.push(data);
        
        if( data.netDifference - transportation > 0 && data.averagePoplarProfit > 0 ) {
          data.adopted = true;
...
function calcAveragePoplarProfit(incomes, costs) {
  var profit = 0, i, t;
  var profits = [];
  
  for( i = 0; i < incomes.length; i++ ) {
    profits.push(incomes[i] - costs[i].cost);
  }
