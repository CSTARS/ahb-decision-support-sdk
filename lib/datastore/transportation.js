...
module.exports = function(DataStore) {
  ...
  DataStore.prototype.getTransportation = function(routeGeometry, callback) {
    ...
    for( var i = 0; i < this.centerPts.length; i++ ) {
      start = [this.centerPts[i][1], this.centerPts[i][0]];
      startingPoints.push(start);
    }


    //this.rest.transportation.getAll(startingPoints, stop, socketId, function(err, resp){
      /// return socket to app
    return this.rest.transportation.getAll(startingPoints, stop, routeGeometry, function(err, resp) {
      if( err ) {
      ...
      for( var i = 0; i < resp.paths.length; i++ ) {
        this.validParcels[i].properties.ucd.transportation = resp.paths[i];
      }
