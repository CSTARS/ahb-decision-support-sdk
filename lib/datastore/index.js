...
function DataStore() {
  ...
  this.getParcels = function(lat, lng, radius, callback) {
    ...
    competingParcels(lat, lng, radius, (parcels) => {
      this.validParcels = parcels;
      this.selectedParcels = [];

      // set a center point
      this.validParcels.forEach((parcel) => {
      ...
      if( callback ) callback(null, parcels);

      this.events.emit('parcels-update-end', parcels);
    }, this.events);
  };
