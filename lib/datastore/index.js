...
function DataStore() {
  ...
  this.validParcelIds = []; // all valid parcels in current range
  ...
  this.setSelectedRefinery = function(name) {
    this.rest.refinery.getAll((refineries) => {
  ...
  this.getAllRefineries = function(callback) {
    this.rest.refinery.getAll(callback);
  }
  
  this.setTotals = function(callback) {
    var totalAcres = 0;
    var totalHarvested = 0;
    var years = this.poplarModel.years;
    var cropCounts = {};

    localdb.find('parcels', 'selected', 1, 
      (parcel, next) => {
  ...
  this.getBudgets = function(callback) {
    ...
    this.rest.budgets.getAll((err, resp) => {
      if( err ) {
