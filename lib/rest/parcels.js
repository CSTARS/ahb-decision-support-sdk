...
var radiusQuery = {
  ...
  inSR : 4326,
  //outFields : '*',
  outFields : 'PolyID, IrrSuitabilityScore, GISAcres, PotentiallySuitPctOfParcel',
  outSR :4326
};
...
function getAll(lat, lng, radius, callback, updatesHandler) {
  ...
  async.eachSeries(
    ...
    function(bbox, next) {
      get(bbox[0], bbox[1], function(err, resp){
        c++;
        console.log(((c/4)*100)+'%');
        if( updatesHandler ) updatesHandler(((c/4)*100));
